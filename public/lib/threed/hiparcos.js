...
function loadStarData( dataFile, callback ){
    ...
    // TODO
    // o.k. this is bad from a memory point of view
    //var loadedData = starsDB.find();
    //console.log("Star count: "+loadedData.count());
    //callback(loadedData.fetch());
}
...
function generateHipparcosStars(){
    ...
    var starPreview = new THREE.Mesh( new THREE.PlaneGeometry( 50, 50 ), starPreviewMaterial );
    ...
    for( var i=0; i<count; i++ ){
        ...
        var z = 200.0; //rho * Math.sin( theta );  //MPN
    ...
    pGeo.attributes = datastarAttributes;
    ...
    //var matrix = new THREE.Matrix4();
    ////	seriously?
    //var euler = new THREE.Euler( 0,0,0); //new THREE.Euler( Math.PI/2,0,0);
    //var quat = new THREE.Quaternion();
    ...
    //starBases.update = function(camera){
    //    // MPN
    ...
    container.update = function(){
        ...
        var brightnessScale = 1; //constrain( 10/ Math.sqrt(camera.position.z), 0, 1);

        // console.log(blueshift);
        if( container.heatVision ){
            datastarUniforms.cameraDistance.value = 0.0;
            datastarUniforms.brightnessScale.value = brightnessScale; //1.0;
        ...
        // MPN: Fix the particle size instead of varying with camera position
        datastarUniforms.zoomSize.value = 0.001;//constrain(( camera.position.z ) / 4000,0,1);
    ...
    lineMesh.update = function(camera){
        //if( camera.position.z < 1500 ){
        //    this.material.opacity = constrain( (camera.position.z - 400.0) * 0.002, 0, 1);
        //}
        //else{
        ...
        //    this.visible = false;
        //else
        //    this.visible = true; //if( camera.position.z < 250 )
        //    this.visible = false;
        //else
        //    this.visible = true;
    };
