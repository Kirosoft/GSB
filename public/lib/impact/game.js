...
ig.Game = class Game {
	...
	update() {
		...
		if (typeof(serverSide) != "undefined") {
			...
			io.emit("entities-kill", this._deferredKill.map(function (ele) {
				return {
					id: ele.id
				}
			}));
	...
	updateEntities() {
		...
		io.emit("entities-update", this.entities.map(function (ele) {
			if (!ele._killed) {
				ele.update();
			}
			return {
